...
// import { generateDataForMinistryOfEducation } from "./controllerMOE.js";
...
//     let data=generateDataForMinistryOfEducation(totalCount, incrementEachLevel, purgeId);

//     // Respond with success
//     res.status(200).json({ message: "Success: Aegis synthetic data generated!",data:data});
//   } catch (error) {
...
const ingestHandler = async (firstLevel, incrementOnEachLevel) => {
	const purgeId = await addPurgeId(purgeNames.MOE);
	const data = generateDataForMinistryOfEducation(purgeId, firstLevel, incrementOnEachLevel);
...
router.post("/ingest", async (req, res) => {
	try {
		...
		const firstLevel = parseInt(req.query.firstLevel) || 100;
...
router.post("/purge", async (req, res) => {
	try {
		const purgeId = await getFirstPurgeId(purgeNames.MOE);

		if (!purgeId) {
			return res.status(404).json({
				status: false,
				message: "No purgeId found for moe",
			});
		}
