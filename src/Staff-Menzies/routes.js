...
import { generateFullStaffData } from "./controllers.js";
...
const ingestHandler = async (firstLevel, incrementOnEachLevel) => {
  const purgeId = await addPurgeId(purgeNames.MENZIESSTAFFDATA);
  const data = generateFullStaffData(purgeId, firstLevel, incrementOnEachLevel);
...
router.post("/purge", async (req, res) => {
  try {
    const purgeId = await getFirstPurgeId(purgeNames.MENZIESSTAFFDATA);

    if (!purgeId) {
      return res.status(404).json({
        status: false,
        message: "No purgeId found for Menzies Staff Data",
      });
    }
...
//     let data = generateFullStaffData(totalCount, incrementEachLevel, purgeId);

//     // Respond with success
//     res.status(200).json({ message: "Success: Menzies synthetic data generated!",data:data});
//   } catch (error) {
