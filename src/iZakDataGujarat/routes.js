...
import { generateiZakGujaratData } from "./controllers.js";
...
const ingestHandler = async (firstLevel, incrementOnEachLevel) => {
  const purgeId = await addPurgeId(purgeNames.IZAKGUJARATDATA);
  const data = generateiZakGujaratData(
    purgeId,
    firstLevel,
...
router.post("/purge", async (req, res) => {
  try {
    const purgeId = await getFirstPurgeId(purgeNames.IZAKGUJARATDATA);
...
//     let data = generateiZakGujaratData(totalCount, incrementEachLevel, purgeId);

//     // Respond with success
//     res.status(200).json({ message: "Success: iZak Gujarat synthetic data generated!",data:data});
//   } catch (error) {
