...
import { generateFullNoorSystemData } from "./controller.js";
...
const ingestHandler = async (firstLevel, incrementOnEachLevel) => {
  const purgeId = await addPurgeId(purgeNames.NOORSYSTEMSDATA);
  const data = generateFullNoorSystemData(
    purgeId,
    firstLevel,
...
router.post("/purge", async (req, res) => {
  try {
    const purgeId = await getFirstPurgeId(purgeNames.NOORSYSTEMSDATA);
...
//     let data = generateFullNoorSystemData(
//       totalCount,
//       incrementEachLevel,
//       purgeId
//     );
