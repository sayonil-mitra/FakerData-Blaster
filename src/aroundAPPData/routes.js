...
import { generateAroundAppData } from "./controllers.js";
...
const ingestHandler = async (firstLevel, incrementOnEachLevel) => {
  const purgeId = await addPurgeId(purgeNames.AROUNDAPPDATA);
  const data = generateAroundAppData(purgeId, firstLevel, incrementOnEachLevel);
...
router.post("/purge", async (req, res) => {
  try {
    const purgeId = await getFirstPurgeId(purgeNames.AROUNDAPPDATA);
...
//     let data = generateAroundAppData(totalCount, incrementEachLevel, purgeId);
