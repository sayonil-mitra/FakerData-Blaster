...
const createGasStationData = (purgeId) => {
  ...
  const randomIndex = faker.number.int({
    min: 0,
    max: gasStationNames.length - 1,
  });

  return {
    station_id: faker.string.uuid(),
    name: gasStationNames[randomIndex],
    ...
    ratings: faker.number.float({ min: 2, max: 5, precision: 0.1 }).toFixed(1),
    logo_url: gasStationslogos[randomIndex],
...
const createEarthquakeData = (purgeId) => {
  ...
  return {
    ...
    magnitude: Number(
      faker.number.float({ min: 1.0, max: 10.0, precision: 0.1 }).toFixed(2)
    ),
    language: faker.helpers.arrayElement(["en"]),
  ...
  switch (eventType) {
    ...
    case "missing_persons":
      ...
      data = {
        ...
        contact_person: faker.helpers.arrayElement([
          "Ahmedabad Police",
          "Harshil",
          "Jinal",
          "Dhwani",
    ...
    case "rentals":
      const minPrice =
        (faker.number.int({ min: 10000, max: 20000 }) / 1000).toFixed(1) + "k";
      const maxPrice =
        (faker.number.int({ min: 20000, max: 30000 }) / 1000).toFixed(1) + "k";
      data = {
        address: `${street}, ${city}`,
...
const createFuelData = (purgeId, stationId) => {
  ...
  // Get a shuffled list of product indices
  const productIndices = faker.helpers
    .shuffle([0, 1, 2, 3])
    .slice(0, numberOfProducts);

  // Generate fuel data for the selected products
  const fuelDataArray = productIndices.map((index) => {
