...
import { generateSchoolData } from "./controller.js";
...
const ingestHandler = async (firstLevel, incrementOnEachLevel) => {
  const purgeId = await addPurgeId(purgeNames.SCHOOLPROGRAM);
  const data = generateSchoolData(purgeId, firstLevel, incrementOnEachLevel);
...
router.post("/purge", async (req, res) => {
  try {
    const purgeId = await getFirstPurgeId(purgeNames.SCHOOLPROGRAM);
...
//     let data = generateSchoolData(totalCount, incrementEachLevel, purgeId);
